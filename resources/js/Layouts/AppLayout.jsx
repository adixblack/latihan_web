...
function NavItem({ href, active, label, hint, disabled }) {
  const base =
    "flex items-center justify-between rounded-2xl px-4 py-3 text-sm font-bold transition";

  if (disabled || !href) {
    return (
      <div className={cx(base, "cursor-not-allowed bg-slate-100 text-slate-400")}>
        <span>{label}</span>
        <span className="text-xs text-slate-400">{hint ?? "soon"}</span>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <span>{label}</span>
      <span className={cx("text-xs", active ? "text-white/70" : "text-slate-400")}>
        {hint ?? (active ? "aktif" : "")}
      </span>
    </Link>
  );
...
export default function AppLayout({
  ...
  const active = (path) => (path ? currentUrl.includes(path) : false);

  const finalNavItems = Array.isArray(navItems) ? navItems : buildMenu(panel);
            ...
            <aside className="lg:col-span-3">
              <div className="rounded-3xl border border-slate-200 bg-white/70 p-4 shadow-sm backdrop-blur">
                ...
                <div className="space-y-2">
                  {finalNavItems.map((it) => (
                    <NavItem
                      key={it.key ?? it.label}
                      href={it.href}
                      active={active(it.activePath)}
                      label={it.label}
                      hint={it.hint}
